// MetallicPaint —— 液态金属效果（WebGL2），自包含组件
// 源码来自桌面《metallic paint食用方法.md》；改良：canvas 适配容器宽高比（不再硬编码正方形）
...
void main(){
  ...
  col=clamp(col,0.,1.);
  // 保留 logo 醒目黄色：G 通道标记为 1 的区域混回黄色（75% 黄 + 25% 金属渐变）
  float accent=texSample.g;
  col=mix(col,vec3(1.,.82,.22),accent*.75);
  col=clamp(col,0.,1.);
  oC=vec4(col*vs,vs);
}`;
...
function processImage(img) {
  ...
  const yellowFlag = new Uint8Array(size); // 保留 logo 的醒目黄色（#fec01a 类）
  ...
  for (let i = 0; i < size; i++) {
    ...
    // 黄色：r 高、g 中高、b 低（如 #fec01a = 254,192,26）
    yellowFlag[i] = r > 200 && g > 140 && b < 120 && !isBackground ? 1 : 0;
  }
  ...
  for (let i = 0; i < size; i++) {
    ...
    outData.data[px] = gray; // R: 深度
    outData.data[px + 1] = yellowFlag[i] * 255; // G: 黄色标记
    outData.data[px + 2] = 0;
...
export default function MetallicPaint({
  ...
  const uploadTexture = useCallback(imgData => {
    ...
    // 改良：u_ratio 用画布实际宽高比（官方硬编码 1=正方形画布）
    const c = canvasRef.current;
    gl.uniform1f(uniforms.u_ratio, c ? c.width / c.height : 1);
  ...
  useEffect(() => {
    ...
    lightColor, darkColor, patternSharpness, waveAmplitude, noiseScale, chromaticSpread, distortion, contour, tintColor]);
