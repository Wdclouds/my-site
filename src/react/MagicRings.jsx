// MagicRings —— 扩散光环（WebGL2 全屏 shader），自包含组件
// 源码来自桌面《magic rings食用方法.md》，无 CSS 依赖（内联样式）
...
export default function MagicRings({
  ...
  propsRef.current = {
    ...
    hoverScale, parallax, clickBurst, onClick,
  };
  ...
  useEffect(() => {
    ...
    const onMountClick = () => { if (propsRef.current.onClick) propsRef.current.onClick(); burstRef.current = 1; };
    ...
    return () => {
      ...
      renderer.dispose();
      material.dispose();
      // 注：不调用 loseContext —— 组件已常驻（display 切换休眠），避免频繁上下文 IPC 开销
    };
  }, []);
