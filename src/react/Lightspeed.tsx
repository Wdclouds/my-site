...
const Lightspeed: React.FC<LightspeedProps> = ({
  ...
  useEffect(() => {
    ...
    const fragmentShader = `
      ...
      float computeStreak(vec2 coord, float timeOffset) {
        ...
        // 彗星拖尾：沿条纹方向（animatedY 越大=越靠尾部）锐利衰减，
        // 让粒子头亮、尾细暗锐。animatedY 0→1，用 (1 - animatedY) 使尾部收敛。
      ...
      void main() {
        ...
        float radius = 2.2 / distFromCenter; // 中心空洞更小、中心粒子更亮（原 2.8）
        ...
        // 距离衰减：加偏移量让中心区域亮度保底（刚射出时不至于全暗）
        // 0.25 偏移使中心 pow(0.35, f) 而非 pow(0.1, f)，亮度提升数倍；外围几乎不受影响
  ...
  const containerClasses = `lightspeed-container ${className}`.trim();
  ...
  return (
    ...
    >
      {children && <div className="lightspeed-children">{children}</div>}
    </div>
  );
};
